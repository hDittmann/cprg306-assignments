    ...
    <div>
      ...
      {ingredient && (
        <div>
          ...
          {meals.length === 0 && <div className="text-gray-400">No meals found for this ingredient.</div>}
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {meals.map((m) => (
              <li key={m.idMeal} className="bg-gray-700 p-3 rounded flex items-center gap-3">
                {m.strMealThumb && (
                  <img src={m.strMealThumb} alt={m.strMeal} className="w-16 h-16 object-cover rounded" />
                )}
